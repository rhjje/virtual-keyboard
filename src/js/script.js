...
let audioSimple = document.createElement('audio');
let sourceSimple = document.createElement('source');
...
let audioSimpleRus = document.createElement('audio');
let sourceSimpleRus = document.createElement('source');
sourceSimpleRus.src = 'assets/sounds/key-simple-rus.wav';
audioSimpleRus.appendChild(sourceSimpleRus);

let audioRus = document.createElement('audio');
let sourceRus = document.createElement('source');
...
const createKeyboard = () => {
    ...
    for (let i = 0; i < sideKeys.length; i++) {
        ...
        for (let j = 0; j < sideKeys[i].length; j++) {
            if (sideKeys[i][j].length === 1) {
                ...
                let spanSideRus = document.createElement('span');
                spanSideRus.classList.add('side-key-rus', 'disabled');

                let spanMainRus = document.createElement('span');
...
let keyShift = document.querySelector('.key.shift');
        ...
        switch (event.target.innerText) {
            ...
            default:
                if (languageEnglish) {
                    ...
                    let span = key.querySelector('.main-key');
                    ...
                    let span = key.querySelector('.main-key-rus');
                    textarea.setRangeText(span.innerText, textarea.selectionStart, textarea.selectionEnd, 'end');
                    textarea.focus();
                }
...
const backspace = () => {
    let firstString = textarea.value.slice(0, textarea.selectionStart);
    let secondString = textarea.value.slice(textarea.selectionEnd);
    let positionFocus = textarea.selectionStart;
...
const shift = () => {
    ...
    keys.forEach(key => {
        if (key.querySelector('.side-key')) {
            let sideKey = key.querySelector('.side-key');
            let mainKey = key.querySelector('.main-key');
            let sideKeyRus = key.querySelector('.side-key-rus');
            let mainKeyRus = key.querySelector('.main-key-rus');

            let temp = mainKey.innerHTML;
            let tempRus = mainKeyRus.innerHTML;
...
const up = () => {
    ...
    textarea.focus();
    if (positionFocus > 0) {
        let count = textarea.value.slice(0, textarea.selectionStart).length;
...
const down = () => {
    ...
    textarea.focus();
    if (positionFocus < textarea.value.length) {
        let count = textarea.value.slice(textarea.selectionEnd).length;
        textarea.setSelectionRange(positionFocus + count, positionFocus + count);
    } else {
        textarea.setSelectionRange(positionFocus, positionFocus);
    }
...
textarea.addEventListener('keydown', (event) => {
    for (let i = 0; i < keysKeyCode.length; i++) {
        if (event.keyCode === keysKeyCode[i] && (i === 42 || i === 55 || i === 57) && event.location === 1) {
            keys[i].classList.add('active');
            break;
        } else if (event.keyCode === keysKeyCode[i] && (i === 54 || i === 59 || i === 63) && event.location === 2) {
            keys[i].classList.add('active');
            break;
